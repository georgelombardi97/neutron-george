...
$BINARY chains add -f $PWD/network/relayer/interchain-acc-config/chains/test-2.json --home $CHAIN_DIR/$RELAYER_DIR

#$BINARY config add-paths $PWD/network/relayer/interchain-acc-config/paths --home $CHAIN_DIR/$RELAYER_DIR
$BINARY paths add test-1 test-2 test1-account-test2 -f $PWD/network/relayer/interchain-acc-config/paths/test1-account-test2.json --home $CHAIN_DIR/$RELAYER_DIR
...
$BINARY tx link test1-account-test2 --home $CHAIN_DIR/$RELAYER_DIR --debug

echo "Starting to listen relayer..."
$BINARY start test1-account-test2 --home $CHAIN_DIR/$RELAYER_DIR --debug
